...
<title>Customer / Apparel Order Form</title>
<head>
...
<label>
<select name="gender" required >
<option selected value="<?php echo $_POST['gender']; ?>" ><?php echo $_POST['gender']; ?></option>
</select>
...
<?php
$SP = mysqli_query($mycon,'SELECT * From `Pages` WHERE `Pages`.`Type` = \'All\' ORDER BY `Pages`.`ID` ASC');
while ( $res = mysqli_fetch_array($SP)){
?>
<option value="<?php echo $res['ID']; ?>"><?php echo $res['Platform']; ?></option>
<?php
}
?>
</select>
...
<label>
<input type="text" name="sss" id="sss"  value="<?php echo $_GET['ID']; ?>"readonly hidden required/>
</label>
<?php
$C = $mycon->query("SELECT * FROM salesperson WHERE SID = '$Z'");
$CR = mysqli_fetch_array ($C);
?>
<label style="font-weight:bolder;color:navy;">Product Model</label>
<label>	
<?php
$SM = mysqli_query($mycon,"SELECT * From `Apparel` Where AID = '$_GET[ID]' ");
$rem = mysqli_fetch_array ($SM)
?>
<input type="text" name="model" value="<?php echo $rem['Model']; ?>" readonly required/>
</label>
<label style="font-weight:bolder;color:navy;">Product Type</label>
<label>	
<input type="text" name="type" value="<?php echo $rem['Type']; ?>" readonly required/>
</label>
<label style="font-weight:bolder;color:navy;">Selling Price Per Unit</label>
<label>
<input type="text" name="price" required value="<?php echo $rem['Price']; ?>"/>
<input type="text" name="momprice" hidden required readonly value="<?php echo $rem['Price']; ?>"/>
<input type="text" name="APS" hidden required readonly value="<?php echo $rem['ActualPrice']; ?>"/>
</label>
<?php
	$Admin = $CR['Admin'];
?>
...
<label>
<input type="text" name="size" value="<?php echo $si;?>" readonly required/>
</label>
...
<label>
<input type="text" name="idseler" value="<?php echo $CR['SID']; ?>" readonly required hidden/>
</label>
<label>
<input type="text" name="sname" value="<?php echo $CR['Sname']; ?>" hidden required/>
</label>
<label>
<input type="submit" name="sub" formaction="ValidateCustApp.php" value="Buy" style="color:deepskyblue;font-weight: bolder;"><i class="icon ion-md-lock"></i>
</label>
<label>
	<a href="Apparel.php?ID=<?php echo $_GET['ID'];?>" style="color:teal;font-weight: bolder;text-decoration:none;">Back To Product</a><i class="icon ion-md-lock"></i>
</label>
<label>
	<a href="Apparels.php" style="color:dodgerblue;font-weight: bolder;text-decoration:none;">Back To All Apparel</a><i class="icon ion-md-lock"></i>
</label>
<label>
	<a href="SellerOrdersApp.php" style="color:blue;font-weight: bolder;text-decoration:none;">View Orders</a><i class="icon ion-md-lock"></i>
